...
Public Class FrmSMGE_3222M_ActiveEnergyImp
    ...
    Private Sub ButtonRecord_Click(sender As Object, e As EventArgs) Handles ButtonRecord.Click
        If ButtonRecord.Text = "Start Recording" Then
            ...
            DataGridViewActiveEnergyImp.Rows.Clear()
            ChartActiveEnergyImpRec.Series("Series1").Points.Clear()
    ...
    Private Sub CmdBack_Click(sender As Object, e As EventArgs) Handles CmdBack.Click
        DataGridViewActiveEnergyImp.Rows.Clear()
        ChartActiveEnergyImpRec.Series("Series1").Points.Clear()
        ChartActiveEnergyImpRec.Series("Series2").Points.Clear()
        ChartActiveEnergyImpRec.Series("Series3").Points.Clear()
        ChartActiveEnergyImpRec.Series("Series4").Points.Clear()
    ...
    Private Sub CmdExportToExcel_Click(sender As Object, e As EventArgs) Handles CmdExportToExcel.Click
        Try
            If DataGridViewActiveEnergyImp.Rows.Count > 0 Then
                ...
                If result = DialogResult.OK Then
                    ...
                    Dim multiselect As Boolean = DataGridViewActiveEnergyImp.MultiSelect
                    DataGridViewActiveEnergyImp.MultiSelect = True
                    DataGridViewActiveEnergyImp.SelectAll()
                    DataGridViewActiveEnergyImp.ClipboardCopyMode = DataGridViewClipboardCopyMode.EnableAlwaysIncludeHeaderText
                    Clipboard.SetDataObject(DataGridViewActiveEnergyImp.GetClipboardContent())
                    Dim results = System.Convert.ToString(Clipboard.GetData(DataFormats.Text))
                    DataGridViewActiveEnergyImp.ClearSelection()
                    DataGridViewActiveEnergyImp.MultiSelect = multiselect
