...
bool ConfigManager::loadWiFiConfig(WiFiConfig& config) {
    ...
    prefs.getBytes("subnet", config.subnet, sizeof(config.subnet));
    prefs.getBytes("dns1", config.dns1, sizeof(config.dns1));
    prefs.getBytes("dns2", config.dns2, sizeof(config.dns2));
...
bool ConfigManager::saveWiFiConfig(const WiFiConfig& config) {
    ...
    prefs.putBytes("subnet", config.subnet, sizeof(config.subnet));
    prefs.putBytes("dns1", config.dns1, sizeof(config.dns1));
    prefs.putBytes("dns2", config.dns2, sizeof(config.dns2));
    
    prefs.end();
    Logger::getInstance().info("WiFi config saved");
    return true;
}
