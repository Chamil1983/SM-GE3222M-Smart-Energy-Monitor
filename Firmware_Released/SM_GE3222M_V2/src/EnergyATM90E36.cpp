 ...
 *****************************************************************************/
void ATM90E3x::DebugIdeMonitor(bool Set, uint8_t BaudRateSelector, uint16_t SerialConfig) {
    if (Set == true) {
        Serial.setTimeout(200);
        switch (BaudRateSelector)
        {
        case BAUD_9600:
            Serial.begin(9600, SerialConfig);     //  Debug Serial
            while (!Serial);
            DBUGS.begin(9600, SerialConfig); // 115200
            while (!DBUGS);
            break;
        case BAUD_19200:
            Serial.begin(19200, SerialConfig);    //  Debug Serial
            while (!Serial);
            DBUGS.begin(19200, SerialConfig); // 115200
            while (!DBUGS);
            break;
        case BAUD_38400:
            Serial.begin(38400, SerialConfig);    //  Debug Serial
            while (!Serial);
            DBUGS.begin(38400, SerialConfig); // 115200
            while (!DBUGS);
            break;
        case BAUD_57600:
            Serial.begin(57600, SerialConfig);    //  Debug Serial
            while (!Serial);
            DBUGS.begin(57600, SerialConfig); // 115200
            while (!DBUGS);
            break;
        case BAUD_115200:
            Serial.begin(115200, SerialConfig);   //  Debug Serial
            while (!Serial);
            DBUGS.begin(115200, SerialConfig); // 115200
            while (!DBUGS);
            break;
        case BAUD_230400:
            Serial.begin(230400, SerialConfig);   //  Debug Serial
            DBUGS.begin(230400, SerialConfig); // 115200
            while (!DBUGS);
            while (!Serial);
            break;
        case BAUD_250000:
            Serial.begin(250000, SerialConfig);   //  Debug Serial
            while (!Serial);
            DBUGS.begin(250000, SerialConfig); // 115200
            while (!DBUGS);
            break;
        case BAUD_500000:
            Serial.begin(500000, SerialConfig);   //  Debug Serial
            while (!Serial);
            DBUGS.begin(500000, SerialConfig); // 115200
            while (!DBUGS);
            break;
        case BAUD_1000000:
            Serial.begin(1000000, SerialConfig);  //  Debug Serial
            while (!Serial);
            DBUGS.begin(1000000, SerialConfig); // 115200
            while (!DBUGS);
            break;
        default:
            Serial.begin(115200, SerialConfig);    //  Debug Serial
...
bool ATM90E3x::calibrationError()
{
    ...
    DBUGS.print("Checksum 0: ");
    DBUGS.print(CS0);
    DBUGS.print("\tChecksum 1: ");
    DBUGS.println(CS1);
    DBUGS.print("Checksum 2: ");
    DBUGS.print(CS2);
    DBUGS.print("\tChecksum 3: ");
    DBUGS.println(CS3);
...
double ATM90E3x::GetLineCurrentCTNSamp()
{
...
//================================================================
double ATM90E3x::GetLineCurrentCT1THDN()
{
    unsigned short current = CommEnergyIC(READ, THDNIA, 0xFFFF);
    return (double)current / 1000;
}
double ATM90E3x::GetLineCurrentCT2THDN()
{
    unsigned short current = CommEnergyIC(READ, THDNIB, 0xFFFF);
    return (double)current / 1000;
}
double ATM90E3x::GetLineCurrentCT3THDN()
{
...
double ATM90E3x::GetMeanActivePowerPhaA()
{
    int val = Read32Register(PmeanA, PmeanALSB);
    return (double)val * 0.00032;
}

double ATM90E3x::GetMeanActivePowerPhaB()
{
    int val = Read32Register(PmeanB, PmeanBLSB);
    return (double)val * 0.00032;
}

double ATM90E3x::GetMeanActivePowerPhaC()
{
    int val = Read32Register(PmeanC, PmeanCLSB);
    return (double)val * 0.00032;
}

double ATM90E3x::GetTotalVSumApparentPower()
{
...
double ATM90E3x::GetMeanActiveFundPowerPhaA()
{
    int val = Read32Register(PmeanAF, PmeanAFLSB);
    return (double)val * 0.00032;
}

double ATM90E3x::GetMeanActiveFundPowerPhaB()
{
    int val = Read32Register(PmeanBF, PmeanBFLSB);
    return (double)val * 0.00032;
}

double ATM90E3x::GetMeanActiveFundPowerPhaC()
{
...
double ATM90E3x::GetMeanActiveHarPowerPhaA()
{
...
double ATM90E3x::GetMeanActiveHarPowerPhaB()
{
...
double ATM90E3x::GetMeanActiveHarPowerPhaC()
{
...
double ATM90E3x::GetMeanReactivePowerPhaA()
{
    int val = Read32Register(QmeanA, QmeanALSB);
    return (double)val * 0.00032;
}

double ATM90E3x::GetMeanReactivePowerPhaB()
{
    int val = Read32Register(QmeanB, QmeanBLSB);
    return (double)val * 0.00032;
}
...
double ATM90E3x::GetMeanReactivePowerPhaC()
{
    int val = Read32Register(QmeanC, QmeanCLSB);
    return (double)val * 0.00032;
}
...
double ATM90E3x::GetMeanApparentPowerPhaA()
{
    int val = Read32Register(SmeanA, SmeanALSB);
    return (double)val * 0.00032;
}

double ATM90E3x::GetMeanApparentPowerPhaB()
{
    int val = Read32Register(SmeanB, SmeanBLSB);
    return (double)val * 0.00032;
}

double ATM90E3x::GetMeanApparentPowerPhaC()
{
    int val = Read32Register(SmeanC, SmeanCLSB);
    return (double)val * 0.00032;
}
...
{
    signed short angleA = (unsigned short)CommEnergyIC(READ, PAngleA, 0xFFFF);
    return (double)angleA / 10;
}
double ATM90E3x::GetPhaseCT2()
{
    signed short angleB = (unsigned short)CommEnergyIC(READ, PAngleB, 0xFFFF);
    return (double)angleB / 10;
}
double ATM90E3x::GetPhaseCT3()
{
    signed short angleC = (unsigned short)CommEnergyIC(READ, PAngleC, 0xFFFF);
    return (double)angleC / 10;
}

//============================================================
double ATM90E3x::GetPhaseangV1()
{
    signed short angleA = (unsigned short)CommEnergyIC(READ, UangleA, 0xFFFF);
    return (double)angleA / 10;
}
double ATM90E3x::GetPhaseangV2()
{
    signed short angleB = (unsigned short)CommEnergyIC(READ, UangleB, 0xFFFF);
    return (double)angleB / 10;
}
double ATM90E3x::GetPhaseangV3()
{
    signed short angleC = (unsigned short)CommEnergyIC(READ, UangleC, 0xFFFF);
    return (double)angleC / 10;
}
...
// **************** GET REGISTER VALUE ****************
unsigned short ATM90E3x::GetValueRegister(unsigned short registerRead)
{
    return (double)CommEnergyIC(READ, registerRead, 0xFFFF); // returns value register
}


unsigned short ATM90E3x::GetValue(unsigned short registerRead)
{
    return (unsigned short)CommEnergyIC(READ, registerRead, 0xFFFF); // returns value register
}
...
unsigned short ATM90E3x::GetRegisterValue(unsigned char address) {
...
double ATM90E3x::GetImportReactiveEnergyPhaC()
{
...
double ATM90E3x::GetImportApparentEnergyPhaA()
{
    unsigned short senergyA = CommEnergyIC(READ, SenergyA, 0xFFFF);
    return (double)senergyA / 100 / 3200; // returns kWh
}

double ATM90E3x::GetImportApparentEnergyPhaB()
{
    unsigned short senergyB = CommEnergyIC(READ, SenergyB, 0xFFFF);
    return (double)senergyB / 100 / 3200; // returns kWh
}

double ATM90E3x::GetImportApparentEnergyPhaC()
{
    unsigned short senergyC = CommEnergyIC(READ, SenergyC, 0xFFFF);
    return (double)senergyC / 100 / 3200; // returns kWh
}

double ATM90E3x::GetImportApparentEnergyArt()
{
    unsigned short saenergyT = CommEnergyIC(READ, SAenergyT, 0xFFFF);
    return (double)saenergyT / 100 / 3200; // returns kWh
}

double ATM90E3x::GetImportApparentEnergyVec()
{
    unsigned short svenergyT = CommEnergyIC(READ, SVenergyT, 0xFFFF);
    return (double)svenergyT / 100 / 3200; // returns kWh
}
...
#endif
{
    ...
    /* Enable SPI */
    SPI.begin(PIN_SPI_SCK, PIN_SPI_MISO, PIN_SPI_MOSI, _energy_CS);

    DBUGS.print("Connecting to the ");

#if ATM90DEVICE == ATM90E32_DEVICE
    DBUGS.println("ATM90E32");
#endif

#if ATM90DEVICE == ATM90E36_DEVICE
    DBUGS.println("ATM90E36");
#endif
...
#if ATM90DEVICE == ATM90E32_DEVICE
    DBUGS.print("ATM90E32");
#endif

#if ATM90DEVICE == ATM90E36_DEVICE
    DBUGS.print("ATM90E36");
#endif

    DBUGS.println(" Connected - OK");
...
#endif
{
   ...
   // _energy_CS = 5;              // SS IPEM = GP5
    _lineFreq = MMode0_Reg; // frequency of power
    _pgagain = MMode1_Reg;   // PGA Gain for current channels#
    ...
    _ugain3 = ugain1; // Duplicate ugain1 Values to _ugain2 and _ugain3.
#else
    _ugain1 = MeasurCalRegisters[0]; // Voltage 1 RMS Gain
    _ugain2 = MeasurCalRegisters[4]; // Voltage 2 RMS Gain
    _ugain3 = MeasurCalRegisters[8]; // Voltage 3 RMS Gain
#endif

    _igainA = MeasurCalRegisters[1]; // CT1
    _igainB = MeasurCalRegisters[5]; // CT2 - not used for single split phase meter
    _igainC = MeasurCalRegisters[9]; // CT3 or CT2 for single split phase meter

#if ATM90DEVICE == ATM90E36_DEVICE && ATM_SPLITPHASE == false
    _igainN = MeasurCalRegisters[12]; // CTN (Used on the ATM90E36)
#endif
    ...
    CommEnergyIC(WRITE, MMode0, _lineFreq);   // 0x1087.  Mode Config (60 Hz, 3P4W)
    CommEnergyIC(WRITE, MMode1, _pgagain);    // 0x1500.  0x5555 (x2) // 0x0000 (1x)
    CommEnergyIC(WRITE, PStartTh, PStartTh_Reg);    // Active Startup Power Threshold
    CommEnergyIC(WRITE, QStartTh, QStartTh_Reg);    // Reactive Startup Power Threshold
    CommEnergyIC(WRITE, SStartTh, SStartTh_Reg);    // Apparent Startup Power Threshold
    CommEnergyIC(WRITE, PPhaseTh, PPhaseTh_Reg);    // Active Phase Threshold
    CommEnergyIC(WRITE, QPhaseTh, QPhaseTh_Reg);    // Reactive Phase Threshold
    CommEnergyIC(WRITE, SPhaseTh, SPhaseTh_Reg);    // Apparent  Phase Threshold
    CommEnergyIC(WRITE, CSZero, 0x4741);      // Checksum 0
#endif
...
#if ATM90DEVICE == ATM90E36_DEVICE
    CommEnergyIC(WRITE, CalStart, 0x5678); // Metering calibration startup
    CommEnergyIC(WRITE, GainA, CalRegisters[6]);    // Line calibration gain
    CommEnergyIC(WRITE, PhiA, CalRegisters[7]);     // Line calibration angle
    CommEnergyIC(WRITE, GainB, CalRegisters[8]);    // Line calibration gain
    CommEnergyIC(WRITE, PhiB, CalRegisters[9]);     // Line calibration angle
    CommEnergyIC(WRITE, GainC, CalRegisters[10]);    // Line calibration gain
    CommEnergyIC(WRITE, PhiC, CalRegisters[11]);     // Line calibration angle
    CommEnergyIC(WRITE, PoffsetA, CalRegisters[0]); // A line active power offset
    CommEnergyIC(WRITE, QoffsetA, CalRegisters[1]); // A line reactive power offset
    CommEnergyIC(WRITE, PoffsetB, CalRegisters[2]); // B line active power offset
    CommEnergyIC(WRITE, QoffsetB, CalRegisters[3]); // B line reactive power offset
    CommEnergyIC(WRITE, PoffsetC, CalRegisters[4]); // C line active power offset
    CommEnergyIC(WRITE, QoffsetC, CalRegisters[5]); // C line reactive power offset
    CommEnergyIC(WRITE, CSOne, 0x0000);    // Checksum 1
#endif
...
#if ATM90DEVICE == ATM90E36_DEVICE
    CommEnergyIC(WRITE, HarmStart, 0x5678); // Metering calibration startup
    CommEnergyIC(WRITE, POffsetAF, HarCalRegisters[0]); // A Fund. active power offset
    CommEnergyIC(WRITE, POffsetBF, HarCalRegisters[1]); // B Fund. active power offset
    CommEnergyIC(WRITE, POffsetCF, HarCalRegisters[2]); // C Fund. active power offset
    CommEnergyIC(WRITE, PGainAF, HarCalRegisters[3]);   // A Fund. active power gain
    CommEnergyIC(WRITE, PGainBF, HarCalRegisters[4]);   // B Fund. active power gain
    CommEnergyIC(WRITE, PGainCF, HarCalRegisters[5]);   // C Fund. active power gain
    CommEnergyIC(WRITE, CSTwo, 0x0000);     // Checksum 2
#endif
  ...
  // A
    ...
    CommEnergyIC(WRITE, UgainA, _ugain1);  // A SVoltage rms gain
    CommEnergyIC(WRITE, IgainA, _igainA);  // A line current gain.
    CommEnergyIC(WRITE, UoffsetA, MeasurCalRegisters[2]); // A Voltage offset
    CommEnergyIC(WRITE, IoffsetA, MeasurCalRegisters[3]); // A line current offset
    // B
    CommEnergyIC(WRITE, UgainB, _ugain2);  // B Voltage rms gain.
    CommEnergyIC(WRITE, IgainB, _igainB);  // B line current gain
    CommEnergyIC(WRITE, UoffsetB, MeasurCalRegisters[6]); // B Voltage offset
    CommEnergyIC(WRITE, IoffsetB, MeasurCalRegisters[7]); // B line current offset
    // C
    CommEnergyIC(WRITE, UgainC, _ugain3);  // C Voltage rms gain
    CommEnergyIC(WRITE, IgainC, _igainC);  // C line current gain
    CommEnergyIC(WRITE, UoffsetC, MeasurCalRegisters[10]); // C Voltage offset
    CommEnergyIC(WRITE, IoffsetC, MeasurCalRegisters[11]); // C line current offset
    // N
    CommEnergyIC(WRITE, IgainN, _igainN); // D line current gain
    CommEnergyIC(WRITE, IoffsetN, MeasurCalRegisters[13]); // D line current gain
    CommEnergyIC(WRITE, CSThree, 0x02F6); // Checksum 3
#endif
